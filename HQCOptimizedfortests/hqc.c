...
void PQCLEAN_HQC128_CLEAN_hqc_pke_keygen(uint8_t *pk, uint8_t *sk) {
    ...
    uint16_t xindexes[PARAM_OMEGA] = {0};
    uint64_t h[VEC_N_SIZE_64] = {0};
    uint64_t s[VEC_N_SIZE_64] = {0};
    uint16_t yindexes[PARAM_OMEGA] = {0};
    ...
    // Compute secret key
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&sk_seedexpander, PARAM_OMEGA, xindexes);
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&sk_seedexpander, PARAM_OMEGA, yindexes);

    // Compute public key
    PQCLEAN_HQC128_CLEAN_vect_set_random(&pk_seedexpander, h);
    PQCLEAN_HQC128_CLEAN_vect_mul_low_weight(s, yindexes, h, PARAM_OMEGA);
    PQCLEAN_HQC128_CLEAN_vect_add_light(s, xindexes, PARAM_OMEGA, VEC_N_SIZE_64);
...
void PQCLEAN_HQC128_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, uint8_t *theta, const uint8_t *pk) {
    ...
    uint64_t h[VEC_N_SIZE_64] = {0};
    uint64_t s[VEC_N_SIZE_64] = {0};
    uint16_t r1indexes[PARAM_OMEGA_R] = {0};
    uint16_t r2indexes[PARAM_OMEGA_R] = {0};
    uint16_t eindexes[PARAM_OMEGA_E] = {0};
    ...
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&vec_seedexpander, PARAM_OMEGA_R, r1indexes);
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&vec_seedexpander, PARAM_OMEGA_R, r2indexes);
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&vec_seedexpander, PARAM_OMEGA_E, eindexes);

    // Compute u = r1 + r2.h
    PQCLEAN_HQC128_CLEAN_vect_mul_low_weight(u, r2indexes, h, PARAM_OMEGA_R);
    PQCLEAN_HQC128_CLEAN_vect_add_light(u, r1indexes, PARAM_OMEGA_R, VEC_N_SIZE_64);
    ...
    PQCLEAN_HQC128_CLEAN_vect_mul_low_weight(tmp2, r2indexes, s, PARAM_OMEGA_R);
    PQCLEAN_HQC128_CLEAN_vect_add_light(tmp2, eindexes, PARAM_OMEGA_E, VEC_N_SIZE_64);
...
void PQCLEAN_HQC128_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, uint8_t *theta, const uint8_t *pk) {
    ...
    uint16_t r1indexes[PARAM_OMEGA_R] = {0};
    uint64_t r2[VEC_N_SIZE_64] = {0};
    uint16_t eindexes[PARAM_OMEGA_R] = {0};
    uint64_t tmp1[VEC_N_SIZE_64] = {0};
    uint64_t tmp2[VEC_N_SIZE_64] = {0};
    ...
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&vec_seedexpander, PARAM_OMEGA_R, r1indexes);
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight(&vec_seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_indexes(&vec_seedexpander, PARAM_OMEGA_R, eindexes);
...
uint8_t PQCLEAN_HQC128_CLEAN_hqc_pke_decrypt(uint8_t *m, uint8_t *sigma, const uint64_t *u, const uint64_t *v, const uint8_t *sk) {
    uint16_t xindexes[PARAM_OMEGA] = {0};
    uint16_t yindexes[PARAM_OMEGA_R] = {0};
    uint8_t pk[PUBLIC_KEY_BYTES] = {0};
    uint64_t tmp1[VEC_N_SIZE_64] = {0};
    uint64_t tmp2[VEC_N_SIZE_64] = {0};

    // Retrieve x, y, pk from secret key
    PQCLEAN_HQC128_CLEAN_hqc_secret_key_from_string(xindexes, yindexes, sigma, pk, sk);

    // Compute v - u.y
    PQCLEAN_HQC128_CLEAN_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);
    PQCLEAN_HQC128_CLEAN_vect_mul_low_weight(tmp2, yindexes, u, PARAM_OMEGA);
    PQCLEAN_HQC128_CLEAN_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_64);
...
uint8_t PQCLEAN_HQC128_CLEAN_hqc_pke_decrypt(uint8_t *m, uint8_t *sigma, const uint64_t *u, const uint64_t *v, const uint8_t *sk) {
    ...
    // Retrieve x, y, pk from secret key
    PQCLEAN_HQC128_CLEAN_hqc_secret_key_from_string(x, yindexes, sigma, pk, sk);
...
uint8_t PQCLEAN_HQC128_CLEAN_hqc_pke_decrypt(uint8_t *m, uint8_t *sigma, const uint64_t *u, const uint64_t *v, const uint8_t *sk) {
    ...
    // Retrieve x, y, pk from secret key
    PQCLEAN_HQC128_CLEAN_hqc_secret_key_from_string(xindexes, y, sigma, pk, sk);
