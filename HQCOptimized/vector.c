...
/**
 * @brief Generates a list of indexes for a vector of a given Hamming weight
 *
 * @param[in] ctx Pointer to the context of the seed expander
 * @param[in] weight Integer that is the Hamming weight
 * @param[in] indexes Pointer to a list of indexes
 */
...
/**
 * @brief Generates a vector of a given Hamming weight and the list of indexes associated
 *
 ...
 * @param[in] weight Integer that is the Hamming weight
 */
void PQCLEAN_HQC128_CLEAN_vect_set_random_fixed_weight_and_index(seedexpander_state *ctx, uint64_t *v, uint16_t weight, uint16_t *indexes) {
    uint8_t rand_bytes[4 * PARAM_OMEGA_R] = {0}; // to be interpreted as PARAM_OMEGA_R 32-bit unsigned ints
    uint32_t support[PARAM_OMEGA_R] = {0};
...
/**
 * @brief Adds a light vector to another
 *
 * @param[out] o Pointer to the result
 * @param[in] indexes Pointer to an array of non zero value indexes
 * @param[in] weight Integer that is the Hamming weight of the light vector
 * @param[in] size Integer that is the size of the resulting vector
 */
void PQCLEAN_HQC128_CLEAN_vect_add_light(uint64_t *o, const uint16_t *indexes, uint16_t weight, uint16_t size) {
  for (uint16_t i = 0; i < weight; ++i) {
    if (indexes[i] >= size*64) {
      break;
    }
    o[(indexes[i]/64)] ^= (1ULL << (indexes[i]%64));
  }
}
